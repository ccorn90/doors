...
namespace ImageUI {
  ...
  extern bool tetherMotion, dualView, quadView;
  extern BBox bbox;

  enum UIMode {NORMAL, ADJUST, ADD_VERTEX, SELECT, CORRESP, LATTICE, PIXELSEG};

  float &imageScale(int imageIndex);
  Coord &windowUL(int imageIndex);
  double getWindowPixelWidth(int imageIndex);
  double getWindowPixelHeight(int imageIndex);
  void addImage(const char *f);
  Image* currImage(int imageIndex=0);
  int currImageWidth(int imageIndex=0);
  int currImageHeight(int imageIndex=0);
  Image *getFaceMap();
  void drawImage(int imageIndex);
  void drawImgBBox();
  bool isValidIndex(int);
  void setImageIndexChar(char, int imageIndex);
  void setImageIndex(int, bool toTitle=false, int imageIndex=0);
  void putImageNameInWindowTitle(int imageIndex);
  void setImage(Image *img, int imageIndex=0);
  void incrImageIndex(bool fwd=true, int imageIndex=0);
  Coord asImageCoord(int x, int y, int imageIndex=0);
  void cropImage();
...
struct CPoly {
  ...
  static void drawAll(int imageIndex=0);
};
...
void zoomInBy(double s, int x, int y, int imageIndex);
void zoomInBy(double s, Coord fixed, int imageIndex);
void resizeWindow(double s, int imageIndex); // in gl.cpp
void doubleWindow();         // in gl.cpp
void quadrupleWindow();         // in gl.cpp
void resetZoom(int imageIndex);
void reshape(int imageIndex);  // in ui.cpp
void reshape(int w, int h, int imageIndex);  // in ui.cpp
void estimateImageScale(int w, int h, float &scale);
void fitWindowToImage(bool updateScale, int imageIndex);
void drawMouseInfo(int x, int y, bool shiftPressed, bool ctrlPressed, int windowIndex);
void undrawMouseInfo();
void drawWindow(int imageIndex=0);
...
pair<Coord,Coord> fitLine2(const vector<Coord> &pt);
